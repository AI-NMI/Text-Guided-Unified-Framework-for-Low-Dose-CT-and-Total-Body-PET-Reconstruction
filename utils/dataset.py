...
class CTDataset(Dataset):
    def __init__(self, dataset, mode, test_id=9, dose=25, context=True):
        self.mode = mode
        ...
        if dataset in ['mayo_2016_sim', 'mayo_2016']:
            if dataset == 'mayo_2016_sim':
                data_root = '/mnt/sdb/i_am_yibo/dataset/CT/mayo2016/mayo_2016_npy'
            ...
            patient_lists = []
            for ind, id in enumerate(patient_ids):
                patient_list = sorted(glob(osp.join(data_root, ('L{:03d}_{}_'.format(id, 25) + '*_img.npy'))))
                if context:
                    cat_patient_list = []
    ...
    def __getitem__(self, index):
        ...
        if self.context:
            ...
            for i in range(1, len(input_path)):
                inputs.append(np.load(input_path[i])[np.newaxis, ...].astype(np.float32))
            input_img = np.concatenate(inputs, axis=0)  #(3, 512, 512)
        else:
            input_img = np.load(input_path)[np.newaxis, ...].astype(np.float32) #(1, 512, 512)
        target_img = np.load(target_path)[np.newaxis,...].astype(np.float32) #(1, 512, 512)
        input_img = self.normalize_(input_img)
        target_img = self.normalize_(target_img)

        # Placeholder text description for text-guided pipelines.
