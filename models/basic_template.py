...
class TrainTask(object):
    ...
    def build_default_options():
        ...
        parser.add_argument('--run_name', type=str, default='default',
                            help='each run name')
        parser.add_argument('--model_name', type=str, default='tuf',
    ...
    def set_loader(self):
        ...
        def parse_sample(sample):
            if isinstance(sample, dict):
                low = sample.get('input')
                full = sample.get('target')
                text = sample.get('text')
                return low, full, text
            if isinstance(sample, (list, tuple)):
                if len(sample) == 2:
                    return sample[0], sample[1], None
                if len(sample) >= 3:
                    return sample[0], sample[1], sample[2]
            raise TypeError(f'Unsupported sample type in dataset: {type(sample)}')

        test_images = [test_dataset[i] for i in range(0, min(300, len(test_dataset)), 75)]
        parsed_samples = [parse_sample(x) for x in test_images]
        low_dose = torch.stack([torch.from_numpy(x[0]) for x in parsed_samples], dim=0).cuda()
        full_dose = torch.stack([torch.from_numpy(x[1]) for x in parsed_samples], dim=0).cuda()
